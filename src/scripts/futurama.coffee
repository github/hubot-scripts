...
module.exports = (robot) ->
  robot.hear /not sure if (.*) or (.*)/i,{id: 'futurama.meme.not.sure'}, (msg) ->
    generateMeme msg, 305, 84688, "not sure if #{ msg.match[1] }", "or #{ msg.match[2] }"

  robot.hear /(.*) is bad and you should feel bad/i,{id: 'futurama.meme.one.feel.bad'}, (msg) ->
    generateMeme msg, 12270, 1136171, "#{ msg.match[1] } is bad", "and you should FEEL bad!"

  robot.hear /(.*) are bad and you should feel bad/i,{id: 'futurama.meme.multiple.feel.bad'}, (msg) ->
    generateMeme msg, 12270, 1136171, "#{ msg.match[1] } are bad", "and you should FEEL bad!"

  robot.hear /(.*) why not zoidberg?/i,{id: 'futurama.meme.zoidberg'}, (msg) ->
    generateMeme msg, 135099, 1643190, msg.match[1], "why not zoidberg?"

  robot.hear /(.*) does not work that way/i,{id: 'futurama.meme.multiple.work.that.way'}, (msg) ->
    generateMeme msg, 2784, 1113725, msg.match[1], "does not work that way"

  robot.hear /(.*) do not work that way/i,{id: 'futurama.meme.one.work.that.way'}, (msg) ->
    generateMeme msg, 2784, 1113725, msg.match[1], "do not work that way"

  robot.hear /.*(hypnotoad).*/i,{id: 'futurama.meme.hypnotoad'}, (msg) ->
    msg.send "http://i0.kym-cdn.com/photos/images/newsfeed/000/008/746/hypnotoadfullsize.gif"

  robot.hear /.*(don'?t want to live).*/i,{id: 'futurama.meme.dont.live'}, (msg) ->
    msg.send "http://i0.kym-cdn.com/photos/images/newsfeed/000/126/314/3cd8a33a.png"

  robot.hear /shut up and take my money/i,{id: 'futurama.meme.my.money'}, (msg) ->
    msg.send "http://i.imgur.com/QlmfC.jpg"

  robot.hear /futurama fry/i,{id: 'futurama.meme.fry'}, (msg) ->
