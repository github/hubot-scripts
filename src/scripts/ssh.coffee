...
send_debug_msgs = /true/i.test(process.env.HUBOT_SSH_PRINT_DEBUG_CONNECTION_INFO)
    ...
    c.on "ready", ->
      ...
      c.exec command, (err, stream) ->
        ...
        stream.on "data", (data, extended) ->
          if debug
            msg.send((if extended is "stderr" then "STDERR: " else "STDOUT: ") + data)
          else
            msg.send("" + data)
        ...
        stream.on "exit", (code, signal) ->
          msg.send("Stream :: exit :: code: " + code + ", signal: " + signal) if debug
          c.end()
